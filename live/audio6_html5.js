...
(function($) {
	...
	function get_wiki_image_history(temp_artist_image,current_obj,options,audio6_html5_container,audio6_html5_thumbsHolder,audio6_html5_thumbsHolderWrapper,audio6_html5_thumbsHolderVisibleWrapper,audio6_html5_historyPadding,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage,audio6_html5_the_wrapper,audio6_html5_historyTitle,the_p) {
					...
					if (options.optional_images_path!='') {
								current_obj.playlist_images_arr[the_p]=temp_artist_image;
					} else {
									/*if (temp_artist_image!='' && temp_artist_image!=undefined) {
			...
			if (current_obj.curTitle!='' && current_obj.isHistoryGenerated) {
				if (current_obj.prevTitle!=current_obj.curTitle) {
					if (current_obj.prevTitle!='') {
						if (current_obj.gen_total_images>=options.historyMaxNumberOfSongs) {
							current_obj.gen_total_images--;
							current_obj.playlist_arr.pop();
							current_obj.playlist_images_arr.pop();
						}
						if (!options.grabLastFmPhoto && !options.azuracast_get_image && !options.mscp_pro_api_url) {
							current_obj.prev_song_image=options.noImageAvailable;
						}
						current_obj.gen_total_images++;
	...
	function get_now_playing(current_obj,options,audio6_html5_thumbsHolder,audio6_html5_thumbsHolderWrapper,audio6_html5_thumbsHolderVisibleWrapper,audio6_html5_container,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage,audio6_html5_historyPadding,audio6_html5_the_wrapper,audio6_html5_historyTitle) {
		if (!options.showOnlyPlayButton && !current_obj.isRadiojar && options.azuracast_api_nowplaying_url=='') {
					if (options.metadata_file_type!='' || options.azuracast_api_nowplaying_url!='') {
								current_obj.now_playing_found=true;
								current_obj.now_playing_current_k=options.metadata_file_type-1;
								if (current_obj.ip=='' || current_obj.ip==undefined) {
										current_obj.ip=1;
								}
					}
					...
					if (options.url_custom_metdata_file!='') {
						my_url=options.url_custom_metdata_file;
					}

					//console.log(my_url+'   --   '+current_obj.now_playing_current_k);
					//alert (my_url);
					var curSong='';
					var startPoint;
					var aux_startPoint;
					var lengthPoint;
					var currentListenersStartPoint;
					var currentListenersLengthPoint;
										...
										switch(current_obj.now_playing_current_k) {
											case 0:
												...
												if (result.indexOf("<SONGTITLE>")!=-1) {
														...
														current_obj.curTitle=current_obj.curTitle.replace(/(\r?\n|\r)/gm, ' ').trim();
														/*currentListeners*/
														currentListenersStartPoint=result.indexOf("<CURRENTLISTENERS>")+18;
														currentListenersLengthPoint=result.indexOf("</CURRENTLISTENERS>")-currentListenersStartPoint;
														current_obj.currentListeners=result.substr(currentListenersStartPoint,currentListenersLengthPoint);
														//console.log(currentListenersStartPoint+' ---  '+currentListenersLengthPoint+'   ---   '+current_obj.currentListeners);
											...
											case 2:
												...
												if (startPoint!==-1) {
														...
														var mount_found=false;
														var the_str='';
														var the_k=0;
														//current_obj.mount_point
														//alert('Obj source length: '+Object.keys(obj_json.icestats.source).length);
														if (obj_json.icestats.source instanceof Array) {
																		while (the_k < Object.keys(obj_json.icestats.source).length && !mount_found) {
																					the_str=obj_json.icestats.source[the_k].listenurl;
																			//alert (the_str+' -- '+the_str.indexOf(current_obj.mount_point));
																			if (the_str.indexOf(current_obj.mount_point) > 0) {
																				mount_found=true;
																				the_k--;
																			}
																			the_k++;
																		...
																		if (mount_found) {
																				/*currentListeners*/
																				if(obj_json.icestats.source[the_k].listeners!='' && obj_json.icestats.source[the_k].listeners!=undefined) {
																						current_obj.currentListeners=obj_json.icestats.source[the_k].listeners;
																				}																			
																				if(obj_json.icestats.source[the_k].title!='' && obj_json.icestats.source[the_k].title!=undefined) {
																						current_obj.now_playing_found=true;
																						curSong=obj_json.icestats.source[the_k].title;
																						current_obj.curTitle=curSong;
																						current_obj.curTitle=current_obj.curTitle.replace(/(\r?\n|\r)/gm, ' ').trim();
																						changeCurrentSongTitle(current_obj,options,audio6_html5_thumbsHolder,audio6_html5_container,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage);
																				}
																...
																/*currentListeners*/
																if(obj_json.icestats.source.listeners!='' && obj_json.icestats.source.listeners!=undefined) {
																	current_obj.currentListenersobj_json.icestats.source.listeners;
																}
																if(obj_json.icestats.source.title!='' && obj_json.icestats.source.title!=undefined) {
																		current_obj.now_playing_found=true;
																		curSong=obj_json.icestats.source.title;
																		current_obj.curTitle=curSong;
																		current_obj.curTitle=current_obj.curTitle.replace(/(\r?\n|\r)/gm, ' ').trim();
																		changeCurrentSongTitle(current_obj,options,audio6_html5_thumbsHolder,audio6_html5_container,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage);
																}
											...
											case 3:
												...
												if (aux_startPoint>0) {
													...
													current_obj.curTitle=current_obj.curTitle.replace(/(\r?\n|\r)/gm, ' ').trim();
													/*currentListeners*/
													aux_startPoint=result.indexOf('Listeners');
													if (aux_startPoint>0) {
														startPoint=result.indexOf('<td class="streamstats">',aux_startPoint);
														if (startPoint>0) {
															startPoint=startPoint+24;
														...
														current_obj.currentListeners=result.substr(startPoint,lengthPoint);
													}
													//console.log(currentListenersStartPoint+' ---  '+currentListenersLengthPoint+'   ---   '+current_obj.currentListeners);
													changeCurrentSongTitle(current_obj,options,audio6_html5_thumbsHolder,audio6_html5_container,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage);
												}
				...
				if (options.azuracast_api_nowplaying_url!='') {
							doCORSRequest({
									method: 'GET',
									url: options.azuracast_api_nowplaying_url+'?my_rand='+Math.random()
							}, function printResult(result) {
									...
									if (startPoint!==-1) {
											...
											var obj_json = JSON.parse(result);
											//console.log(obj_json.now_playing.song.text);
											//alert (Object.keys(obj_json).length+'  --  '+obj_json['title']);
											current_obj.curTitle=obj_json.now_playing.song.text;
											current_obj.curTitle=current_obj.curTitle.replace(/(\r?\n|\r)/gm, ' ').trim();
											if (options.azuracast_get_image) {
													current_obj.azuracast_cur_song_image=options.noImageAvailable;
													if (obj_json.now_playing.song.art!='') {
															current_obj.azuracast_cur_song_image=obj_json.now_playing.song.art;
															//console.log(current_obj.azuracast_cur_song_image);
													}
													current_obj.prev_song_image=current_obj.azuracast_cur_song_image;
											}
											changeCurrentSongTitle(current_obj,options,audio6_html5_thumbsHolder,audio6_html5_container,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage);
				        ...
				        if (data.hasOwnProperty('artist') && data.artist.hasOwnProperty('image')) {
				              //alert ('data: '+data.artist.image[3]['#text'].trim());
										...
										current_obj.musicbrainzHistory_setTimeout_arr[the_p]=setTimeout(function(){
															...
															///console.log("!!!!!!!!!!!!!!!!---------history--------------!!!!!!!!!!!!!!!!!!!!!!!");
															if (options.optional_images_path!='') {
																			temp_artist_image=options.optional_images_path+current_obj.playlist_arr[the_p]+'.jpg';
																			//temp_artist_image=options.optional_images_path+'madalina'+'.jpg';
															...
															if (photo_path!=options.noImageAvailable) {
																				get_wiki_image_history(temp_artist_image,current_obj,options,audio6_html5_container,audio6_html5_thumbsHolder,audio6_html5_thumbsHolderWrapper,audio6_html5_thumbsHolderVisibleWrapper,audio6_html5_historyPadding,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage,audio6_html5_the_wrapper,audio6_html5_historyTitle,the_p);
															}	else {
																			...
																			var now_artist_name=author_arr[0];
																			now_artist_name=now_artist_name.toLowerCase();
																			now_artist_name=removeAccents(now_artist_name);
																			//console.log('history '+the_p+': '+now_artist_name);
																			$.get( "https://theaudiodb.com/api/v1/json/"+current_obj.zerx+"/search.php?s="+author_arr[0], {}, function( result ) {
																						...
																						get_wiki_image_history(temp_img,current_obj,options,audio6_html5_container,audio6_html5_thumbsHolder,audio6_html5_thumbsHolderWrapper,audio6_html5_thumbsHolderVisibleWrapper,audio6_html5_historyPadding,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage,audio6_html5_the_wrapper,audio6_html5_historyTitle,the_p);
																			});
	...
	function changeSrc(current_obj,options,audio6_html5_thumbsHolder,audio6_html5_container,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage,audio6_html5_thumbsHolderVisibleWrapper,audio6_html5_thumbsHolderWrapper,audio6_html5_historyPadding,audio6_html5_the_wrapper,audio6_html5_historyTitle,audio6_html5_minimize_btn) {
			  clearInterval(current_obj.radioReaderAjaxInterval);
				...
				if (!options.showOnlyPlayButton) {
													...
													doCORSRequest({
															...
															for (i = 0; i < Object.keys(obj_json).length; i++) {
																	if (obj_json[i]['track']!='' && obj_json[i]['track']!='Empty Title') {
																		...
																		//console.log(p+'   ---   '+'   --------      '+current_obj.playlist_arr[p]+'   --------      '+current_obj.playlist_images_arr[p])
							...
							if (options.azuracast_api_nowplaying_url!='') { //only for azuracast
												doCORSRequest({
														method: 'GET',
														url: options.azuracast_api_nowplaying_url+'?my_rand='+Math.random()
												}, function printResult(result) {
														...
														if (startPoint!==-1) {
																...
																//console.log(result);
																	...
																	var obj_json = JSON.parse(result);
																	//console.log(obj_json.song_history);
																	//console.log(Object.keys(obj_json.song_history).length);
																	for (i = 0; i < Object.keys(obj_json.song_history).length; i++) {
																			//console.log('song: '+obj_json.song_history[i]['song']['text']);
																			if (obj_json.song_history[i]['song']['text']!='' && obj_json.song_history[i]['song']['text']['text']!='Empty Title') {
																				p++;
																				current_obj.playlist_arr[p]=obj_json.song_history[i]['song']['text'];
																				current_obj.playlist_images_arr[p]=options.noImageAvailable;
																				if (options.azuracast_get_image) {
																						//console.log(obj_json.song_history[i]['song']['text']+'    ----   '+obj_json.song_history[i]['song']['art']);
																						if (obj_json.song_history[i]['song']['art']!='') {
																								current_obj.playlist_images_arr[p]=obj_json.song_history[i]['song']['art'];
																						}
																				}
																				history_lastfm_call(current_obj,options,audio6_html5_thumbsHolder,audio6_html5_container,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage,audio6_html5_thumbsHolderVisibleWrapper,audio6_html5_thumbsHolderWrapper,audio6_html5_historyPadding,audio6_html5_the_wrapper,audio6_html5_historyTitle,p);
																			}
							...
							if (!current_obj.isRadiojar && options.azuracast_api_nowplaying_url=='' && !options.iceCastFirst) { //general
													doCORSRequest({
														  method: 'GET',
																				 ...
																				 //alert (history_aux_arr[i]);
																				  if (history_aux_arr[i]!='' && history_aux_arr[i]!='Empty Title') {
																					  ...
																					  current_obj.playlist_arr[p]=history_aux_arr[i];
																						current_obj.playlist_arr[p]=current_obj.playlist_arr[p].replace(/(\r?\n|\r)/gm, ' ').trim();
																					  current_obj.playlist_images_arr[p]=options.noImageAvailable;
																					  history_lastfm_call(current_obj,options,audio6_html5_thumbsHolder,audio6_html5_container,audio6_html5_play_btn,audio6_html5_Author,audio6_html5_Title,audio6_html5_TitleInside,audio6_html5_radioStation,audio6_html5_Audio,audio6_html5_ximage,audio6_html5_thumbsHolderVisibleWrapper,audio6_html5_thumbsHolderWrapper,audio6_html5_historyPadding,audio6_html5_the_wrapper,audio6_html5_historyTitle,p);
																				  }
																			  }
...
//current_obj.curTitle="A-HA - Bed Of Roses";
				...
				var radioStation_text='';
				if (options.showRadioStation && options.radio_name!=null && options.radio_name!='') {
					radioStation_text=options.radio_name;
				}
				if (options.showListeners && current_obj.currentListeners>0 && current_obj.currentListeners!=null && current_obj.currentListeners!='') {
					radioStation_text+=` (${current_obj.currentListeners} ${options.translateListeners})`;
				}
				
				audio6_html5_radioStation.html(radioStation_text);
				...
				if (options.grabLastFmPhoto && author_arr[0].trim()!='') {
						...
						/*if (author_arr[0]!='' && author_arr[0]!=undefined && current_obj.prevTitle!=current_obj.curTitle) {
										...
										current_obj.musicbrainz_setTimeout=setTimeout(function(){
											if (options.optional_images_path!='') {
															temp_artist_image=options.optional_images_path+current_obj.curTitle+'.jpg';
															temp_artist_image=encodeURI(temp_artist_image);
															//console.log(temp_artist_image);
															//temp_artist_image=encodeURIComponent(temp_artist_image);
															//temp_artist_image="https://christunveiled.org/coverart/Isabel%20Davis%20-%20Wide%20as%20the%20Sky%20(Live).jpg";
											...
											} //else options.optional_images_path!=''
										}, 1500);
						}*/
										...
										current_obj.musicbrainz_setTimeout=setTimeout(function(){
											...
											if (photo_path!=options.noImageAvailable) {
															...
															$.get( "https://theaudiodb.com/api/v1/json/"+current_obj.zerx+"/search.php?s="+author_arr[0], {}, function( result ) {
																		...
																		changeArtistImage(options,current_obj,audio6_html5_ximage,temp_img);
															});
											}
				...
				} else { //options.grabLastFmPhoto=false
						if (options.azuracast_get_image) {
								photo_path=current_obj.azuracast_cur_song_image;
								//console.log('photo_path x:'+photo_path);
								changeArtistImage(options,current_obj,audio6_html5_ximage,photo_path);
						}
						if (options.mscp_pro_api_url!='' && current_obj.prevTitle!=current_obj.curTitle) {
								doCORSRequest({
										method: 'GET',
										url: options.mscp_pro_api_url
								}, function printResult(result) {
										//console.log(result);
										var startPoint=result.indexOf('{"mscp":');
										var lengthPoint=result.length;
										if (startPoint!==-1) {
												result=result.substr(startPoint,lengthPoint);

												var mscp_pro_obj_json = JSON.parse(result);
												var mscp_pro_mount_found=false;
												//console.log(mscp_pro_obj_json.mscp.info[0].cover+'   ----  '+mscp_pro_obj_json.mscp.info[0].cover.indexOf('nocover.png'));
												if (mscp_pro_obj_json.mscp.info[0].cover!='' && mscp_pro_obj_json.mscp.info[0].cover.indexOf('nocover.png')==-1) {
														photo_path=mscp_pro_obj_json.mscp.info[0].cover.replace(/(\s)/g, '%20');
														photo_path=mscp_pro_obj_json.mscp.info[0].cover.replace(/100x100/g, '400x400');
												}
												changeArtistImage(options,current_obj,audio6_html5_ximage,photo_path);
										}
								});
	...
	$.fn.audio6_html5 = function(options) {
		...
		return this.each(function() {
			...
			var current_obj = {
				...
				cur_song_image:'',
				prev_song_image:'',
				azuracast_cur_song_image:options.noImageAvailable,
				curSongText:'',
				curSongAuthorText:'',
				...
				zerx:'523532',
				html5_audio_tag:'',
				audioID:'hero_audio_tag_id_'+randNo,
				currentListeners:0
			};

			if (options.azuracast_get_image && options.azuracast_api_nowplaying_url!='') {
					options.grabLastFmPhoto=false;
			}

			if (options.mscp_pro_api_url!='') {
					options.grabLastFmPhoto=false;
			}

			if (options.iceCastFirst) {
				current_obj.now_playing_current_k=1;
				current_obj.isHistoryGenerated=true;
			}
			...
			audio6_html5_container.append(current_obj.html5_audio_tag);
			...
			//chrome and safari on mac auto-play restrictions 2018 + firefox 2019 end
